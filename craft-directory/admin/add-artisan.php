...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $stmt = $conn->prepare("INSERT INTO artisans (name,craft_id,region_id,experience_years,workshop_address,contact_number,email,description,technique,is_verified) VALUES (?,?,?,?,?,?,?,?,?,?)");
    $v = isset($_POST['is_verified']) ? 1 : 0;
    $stmt->bind_param("siiisssssi",
        $_POST['name'], $_POST['craft_id'], $_POST['region_id'], $_POST['experience_years'],
        $_POST['workshop_address'], $_POST['contact_number'], $_POST['email'],
        $_POST['description'], $_POST['technique'], $v);
    $stmt->execute();
    setFlash('success', 'Artisan added successfully.');
  ...
  <form method="POST" class="contact-form" style="max-width:780px;">
    ...
    <select name="craft_id" required
      ...
      <?php while($c=$crafts->fetch_assoc()): ?>
        <option value="<?= $c['id'] ?>"><?= e($c['craft_name']) ?></option>
      <?php endwhile; ?>
    </select>
    ...
    <input type="text" name="workshop_address" placeholder="Workshop Address" required>
    <textarea name="description" rows="4" placeholder="Artisan Background / Description" required></textarea>
    <textarea name="technique" rows="3" placeholder="Technique and Materials Used" required></textarea>

    <label style="display:flex;align-items:center;gap:10px;margin-bottom:20px;font-size:14.5px;color:var(--indigo);">
      <input type="checkbox" name="is_verified" value="1" style="width:auto;margin:0;">
      Mark as Verified
    </label>

    <button class="btn btn-primary">Save Artisan</button>
